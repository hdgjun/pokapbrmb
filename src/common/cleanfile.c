...
void *CleanFileThread() {
	...
	while (1) {
		...
		sprintf(DelQcfStr,
				"find %s/log/ -name *.txt -type f -ctime +%d -exec rm -f {} \\;>/dev/null 2>&1 ",
				apppath, g_param.LogSaveDay);
#ifdef DEBUG
		...
		if (g_param.SaveImage == DEF_IMAGE_SAVE)
		{
			...
			sprintf(DelQcfStr, "sh %s/%s/%s %s/%s/ %d", apppath, SHELL_DIR,
					DELETE_IMAGE_SHELLNAME_STRING, g_param.ImagePath,
					g_param.ImageDir, idate);
#ifdef DEBUG
			vLog("***cmd[%s]", DelQcfStr);
#endif
		...
		sprintf(DelQcfStr, "sh %s/%s/%s %s/%s/ %d", apppath, SHELL_DIR,
				DELETE_FSN_SHELLNAME_STRING, g_param.FileStoreBasePath,
				g_param.handleDir, idate);
#ifdef DEBUG
		vLog("***cmd[%s]", DelQcfStr);
#endif
